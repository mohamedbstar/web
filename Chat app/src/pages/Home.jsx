...
const Home = () => {
  ...
  //when keyword press escape ==> curChat = null, curUserToChatWith = null, setCurChatMsgs = []
  useEffect(()=>{
              ...
              <div className='msgs w-[30%] h-[100%] bg-slate-100 flex flex-col p-2'>
                      ...
                      <ul className='flex flex-col list-none p-1 w-[90%]'>
                        {
                          searchSuggestions.map((s) => {
                            return (
                              ...
                              }} className='border border-2 p-2 bg-green-200 hover:bg-green-400 text-slate-700 rounded-xl font-semibold cursor-pointer'>{s.name}</li>
                            )
                          })
                        }
                      </ul>
                ...
                <div className='overflow-y-scroll mt-2'>
                  ...
                  {/* render chats here */ chats.length > 0 &&
                    chats.map((c) => {
                      return (
                        <div className={`w-[100%] py-2 my-2 bg-white ps-2 rounded-lg cursor-pointer relative ${curChat == c ? 'bg-red-100' : ''}`} onClick={() => {
                          setCurChat(c);
                          setCurUserToChatWith(
                            c.members[0] == CurrentUser.email ? c.members[1] : c.members[0]
                          )
                          chatEndRef.current?.scrollIntoView();
                          console.log('scrolling from onClick');
                          ...
                          {
                            c.lastMessage && c.lastMessage.from != CurrentUser.email &&!c.lastMessage.seen && <div className={`w-2 h-2 rounded-full bg-red-500 absolute top-1 right-2`}>
                            </div>
                          }
                        </div>
                      )
              ...
              <div className='chat w-[70%] h-[100%]'>
                ...
                {curChat &&
                  <div className='relative h-[100%] w-[100%]'>
                    <div id='chat-msgs' onClick={() => {
                      if (curChat.lastMessage) {
                        console.log('setting kastmessage to true');
                        chatEndRef.current?.scrollIntoView();
                        curChat.lastMessage.seen = true
                        setDummy(!dummy);
                      }
                    }} className='chat-msgs bg-slate-50 w-[98%] h-[90%] rounded-md relative top-2 mx-auto flex flex-col overflow-scroll items-end'>
